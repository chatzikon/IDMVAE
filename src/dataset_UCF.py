...
class UCFDataset(Dataset):
    """
    PyTorch Dataset for UCF producing (image, onehot_caption) pairs.

    Data files in datadir:
      - images.pt             # tensor [N,3,256,256]
      - captions.pt: list of N variable-length lists of raw captions
    ...
    def __init__(self, datadir, split='train', cluster_only=False, transform=None, use_pretrain_feats=True,
        ...
        if shared_data is not None:
            ...
            self.labels_color=shared_data["labels_color"]
        ...
        for img_idx in raw_indices:
            ...
            for cap_idx in range(len(self.captions[img_idx])):
                ...
                # cap_idx ranges from 0 to len(self.captions[img_idx]) - 1
                self.pairs.append((img_idx, cap_idx)) # Each image can have a variable number of captions.
...
class UCF_pregen_4x32x32_7x(UCFDataset):
    ...
    fewer than seven captions leave the remaining slots unused.
    """
    def __init__(self, datadir, split='train', cluster_only=False,
        ...
        if not isinstance(self.captions, list) or not self.captions:
            raise ValueError(
                "captions must be a non-empty list of per-image caption lists"
            )

        caption_counts = [
            len(image_captions)
            for image_captions in self.captions
        ]

        max_captions_per_image = max(caption_counts)

        input_latent_slots = self.latent_inputs.shape[1]
        output_latent_slots = self.latent_outputs.shape[1]

        if input_latent_slots != output_latent_slots:
            raise ValueError(
                "Input and output latent tensors have different numbers "
                f"of latent slots: inputs={input_latent_slots}, "
                f"outputs={output_latent_slots}"
            )

        if input_latent_slots < max_captions_per_image:
            raise ValueError(
                "There are not enough pre-generated latent samples for "
                "the image with the most captions: "
                f"latent_slots={input_latent_slots}, "
                f"max_captions_per_image={max_captions_per_image}"
            )
...
def main_pregen_cub() -> None:
    ...
    captions_for_generation = captions[:num_samples]

    max_captions_per_image = max(
        len(image_captions)
        for image_captions in captions_for_generation
    )

    if samples_per_image < max_captions_per_image:
        raise ValueError(
            "samples_per_image is smaller than the maximum number "
            "of captions per image: "
            f"samples_per_image={samples_per_image}, "
            f"max_captions_per_image={max_captions_per_image}"
        )
