...
from dataset_CUBcluster8 import load_cub_i2w
...
def _plot_sentences_as_tensor(model, batched_text_modality, i2w=None):
    if i2w is None:
        i2w = load_cub_i2w(
            ...
            getattr(model.params, "cub_vocab_min_occ", 3),
        )
...
def train_clf_lr_CUB_multi_labelTypes(model, dl, device, args, condition_type=None):
    ...
    for dataT_lr in dl:
        ...
        if condition_type is None:
            ...
            direction_labels_tensor = labels_batch[1]
            mask = (direction_labels_tensor == 0) | (direction_labels_tensor == 1)
            labels = direction_labels_tensor[mask].cpu().data.numpy()
...
def linear_latent_classification_CUB_multi_labelTypes(model, test_loader, clf_lr, device, args, condition_type=None):
    ...
    model.eval()
    ambiguous_image_ids = {3413, 8172, 10256, 11432, 5591, 482, 4287, 8468}
    lr_acc_all_z = []
    ...
    with torch.no_grad():
        for dataT in test_loader:
            ...
            cluster_lbls, dir_lbls, cat_lbls, img_ids, dataset_indices = targets
            base_validity_mask = (
                (dir_lbls == 0) | (dir_lbls == 1)
                if condition_type == "private"
                else torch.ones_like(img_ids, dtype=torch.bool)
            )
            manual_ambiguous_mask = torch.from_numpy(np.isin(img_ids.cpu().numpy(), list(ambiguous_image_ids))).to(device)
            ...
            for is_ambiguous, mask in [(False, valid_mask), (True, ambiguous_mask)]:
                ...
                labels_batch_for_clf = dir_lbls[mask] if condition_type == "private" else cluster_lbls[mask]
                if clf_lr is None:
                    continue
                latent_reps = []
                for v, vae in enumerate(model.vaes):
                ...
                collected_data["category_labels"].append(cat_lbls[mask].cpu().numpy())
                collected_data["cluster_labels"].append(cluster_lbls[mask].cpu().numpy())
                collected_data["direction_labels"].append(dir_lbls[mask].cpu().numpy())
    ...
    if collected_data:
        ...
        if condition_type in ["shared", "private"]:
            ...
            key_name = "cluster" if condition_type == "shared" else "direction"
            ...
            for _, row in top_x_per_class.iterrows():
                ...
                sample_data = {
                    ...
                    "direction_label": final_data["direction_labels"][idx],
...
def save_images_with_labels(imgs, labels_tuple, save_dir, prefix="orig"):
    ...
    lbl_cluster, lbl_dir, lbl_cat, img_id, dataset_index = labels_tuple
    for i in range(imgs.size(0)):
        fname = (
            f"{prefix}_clu_{int(lbl_cluster[i])}"
            f"_cat_{int(lbl_cat[i])}"
            f"_dir_{int(lbl_dir[i])}"
            f"_id_{int(img_id[i])}"
