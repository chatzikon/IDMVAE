...
from dataset_UCF import UCF_pregen_4x32x32_1x
...
def main(args):
    ...
    if any([args.text2img_qzpw, args.img2text_qzpw, args.img2img_qzpw, args.img2img_qwpz, args.img_random,
            args.text_random]):
    ...
    jobs = []
    if not any([args.write_orig_image, args.text2img_qzpw, args.img2text_qzpw, args.img2img_qzpw, args.img2img_qwpz,
                args.img_random, args.text_random]):
    ...
    for data, _ in tqdm(loader, desc=iter_desc):
        ...
        # ============================================================
        # TEST 2b: Text -> Text using POSTERIOR MEAN (no sampling)
        # ============================================================

        captions_device = captions.to(device)

        with torch.no_grad():

            # Encoder gives the parameters of q(u | text)
            qu_params = text_vae.enc(captions_device)

            # Construct posterior distribution
            qu_text = text_vae.qu_x(*qu_params)

            # Use posterior mean instead of rsample()
            latents_mean = qu_text.mean.unsqueeze(0)

            # Decode the deterministic mean latent
            px_txt_mean = text_vae.px_u(
                *text_vae.dec(latents_mean)
            )

            recon_txt_mean = (
                get_mean(px_txt_mean)
                .squeeze(0)
                .cpu()
            )

        for i in range(min(5, recon_txt_mean.shape[0])):

            gen_cap = recon_txt_mean[i].numpy()
            ...
            for j in range(gen_cap.shape[0]):
                ...
                if tok == dataset.eos_token:
                    break

                if tok != dataset.pad_token:
                    words.append(tok)

            print(f"[TEXT->TEXT MEAN {i}] {' '.join(words)}")
        ...
        for job in jobs:
            ...
            if job == "denoise_eval":
                ...
                with torch.no_grad():
                    p_w = resnet_model.pws_diffusion[
                        0] if args.use_diffusion_prior else resnet_model.get_simple_prior_w(view=0, aux=False)
                    _, latents_z = torch.split(text_us,
                                               [resnet_model.params.latent_dim_w, resnet_model.params.latent_dim_z],
                                               dim=-1)  # (1,B,Z+W)
                    ...
                    denoised_latent = diffusion.p_sample_loop(
                        model.forward, z0.shape, z0, clip_denoised=False, model_kwargs=model_kwargs, progress=False,
                        device=device
                    )
                ...
                with torch.no_grad():
                    latents_w_img, latents_z_img = torch.split(img_us, [resnet_model.params.latent_dim_w,
                                                                        resnet_model.params.latent_dim_z],
                                                               dim=-1)  # (1,B,Z+W)
                    p_w_text = resnet_model.pws_diffusion[
                        1] if args.use_diffusion_prior else resnet_model.get_simple_prior_w(view=1, aux=False)
                    # import pdb; pdb.set_trace()
                ...
                with torch.no_grad():
                    p_w_img = resnet_model.pws_diffusion[
                        0] if args.use_diffusion_prior else resnet_model.get_simple_prior_w(view=0, aux=False)
                    _, latents_z_img = torch.split(img_us,
                                                   [resnet_model.params.latent_dim_w, resnet_model.params.latent_dim_z],
                                                   dim=-1)
                ...
                with torch.no_grad():
                    p_z = resnet_model.pz_diffusion if args.use_diffusion_prior and hasattr(resnet_model,
                                                                                            "pz_diffusion") else resnet_model.get_simple_prior_z()
                    latents_w_img, _ = torch.split(img_us,
                                                   [resnet_model.params.latent_dim_w, resnet_model.params.latent_dim_z],
                                                   dim=-1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--high-res-data-path", type=str,
                        default="/data/backed_up/shared/Data/CUB/CUBcluster8_256/cats22_256px_70_15_15_nonbbox")
