...
sentenceBottleneck = maxSentLen // 8      # 6
embeddingBottleneck = embeddingDim // 8  # 16
...
class Enc(nn.Module):
    ...
    def __init__(self, latentDim_w, latentDim_z, dist, vocab_size=1590):
        ...
        self.c1_w = nn.Linear(fBase * 4 * sentenceBottleneck * embeddingBottleneck,latentDim_w)
        self.c2_w = nn.Linear(fBase * 4 * sentenceBottleneck * embeddingBottleneck,latentDim_w)

        self.c1_z = nn.Conv2d(fBase * 16,latentDim_z,(sentenceBottleneck, 4),1,0,bias=True)

        self.c2_z = nn.Conv2d(fBase * 16,latentDim_z,(sentenceBottleneck, 4),1,0,bias=True
        )
...
class Dec(nn.Module):
    ...
    def __init__(self, latentDim_w, latentDim_z, vocab_size=1590):
        ...
        self.dec_w = nn.Sequential(
            nn.ConvTranspose2d(latentDim_w, fBase * 16, (sentenceBottleneck, 4), 1, 0, bias=True),
            nn.BatchNorm2d(fBase * 16),
            nn.ReLU(True),
            # size: (fBase * 8) x 4 x 4
        ...
        self.dec_z = nn.Sequential(
            nn.ConvTranspose2d(latentDim_z, fBase * 16, (sentenceBottleneck, 4), 1, 0, bias=True),
            nn.BatchNorm2d(fBase * 16),
            nn.ReLU(True),
            # size: (fBase * 8) x 4 x 4
            nn.ConvTranspose2d(fBase * 16, fBase * 8, (1, 4), (1, 2), (0, 1), bias=True),
