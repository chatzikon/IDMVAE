...
class EncoderImg(nn.Module):
    ...
    def __init__(self, ndim_w, ndim_z, dist, img_size=64, in_channels=3):
        super().__init__()
        self.dist = dist
        s0 = self.s0 = 2  # kwargs['s0']  # new_release: 2
        nf = self.nf = 64  # nfilter
        nf_max = self.nf_max = 1024  # nfilter_max  # new_release: 256
        ...
        self.conv_img_w = nn.Conv2d(in_channels, 1*nf, 3, padding=1)
        ...
        self.conv_img_z = nn.Conv2d(in_channels, 1 * nf, 3, padding=1)
    ...
    def forward(self, x):
        ...
        if self.dist == 'Normal':
            return torch.cat((mu_w, mu_z), dim=-1),\
               torch.cat((F.softplus(lv_w).squeeze() + eps, F.softplus(lv_z).squeeze() + eps), dim=-1)
        else:
            return torch.cat((mu_w, mu_z), dim=-1), \
                   torch.cat((F.softmax(lv_w, dim=-1) * lv_w.size(-1) + eps, F.softmax(lv_z, dim=-1) * lv_z.size(-1) + eps),
                             dim=-1)
...
class DecoderImg(nn.Module):
    ...
    def __init__(self, ndim, img_size=64, out_channels=3):
        ...
        self.conv_img = nn.Conv2d(nf, out_channels, 3, padding=1)
