...
def first_pair_position_per_image(dataset_indices):
    ...
    for pair_position, image_index in enumerate(dataset_indices):
        image_index = int(image_index)

        if image_index not in seen_images:
            seen_images.add(image_index)
            first_positions.append(pair_position)

    return first_positions

from eval_functions_UCF import (
...
from dataset_UCF import UCFDataset, CUBImageViewDataset, CUBCaptionViewDataset
...
parser.add_argument('--inception_path', type=str,
                    default='/data/backed_up/shared/Data/CUB/pt_inception-2015-12-05-6726825d.pth',
...
parser.add_argument('--dataset', type=str, default='UCF', choices=['UCF', 'CUB', 'CUBICC', 'CUBcluster8', 'CUBcluster8_256'],
                    help='dataset choice')
...
if args.resume:
    ...
    if os.path.exists(runPath):
        print(
            f"Error: Run path '{runPath}' already exists. Use --resume to continue or change parameters to start a new run.")
        sys.exit(1)
...
if args.dataset == 'UCF':
    ...
    shared_data = {
        "images": torch.load(
            os.path.join(base_dir, "images.pt"),
            weights_only=False,
            map_location="cpu",
        ),

        "captions": torch.load(
            os.path.join(base_dir, "captions.pt"),
            weights_only=False,
            map_location="cpu",
        ),

        # Compatibility alias for the original CUB code.
        # For UCF, this is identical to labels_category.
        "labels_cluster": torch.load(
            os.path.join(base_dir, "labels_cluster.pt"),
            weights_only=False,
            map_location="cpu",
        ),

        # Public/shared semantic factor: crime class.
        "labels_category": torch.load(
            os.path.join(base_dir, "labels_category.pt"),
            weights_only=False,
            map_location="cpu",
        ),

        # Image-private evaluation label:
        # 0 = grayscale, 1 = RGB.
        "labels_color": torch.load(
            os.path.join(base_dir, "labels_color.pt"),
            weights_only=False,
            map_location="cpu",
        ),

        "image_ids": torch.load(
            os.path.join(base_dir, "image_ids.pt"),
            weights_only=False,
            map_location="cpu",
        )
        if os.path.exists(
    ...
    val_cluster_dataset = UCFDataset(
        datadir=base_dir,
        split='val',
        transform=None,
        use_pretrain_feats=use_pretrain_feats,
    ...
    test_cluster_dataset = UCFDataset(
        datadir=base_dir,
        split='test',
        transform=None,
        use_pretrain_feats=use_pretrain_feats,
    ...
    with torch.no_grad():
        ...
        test_selected_samples, labels_list_of_tuples, \
            ...
            get_test_CUBcluster8_samples(test_time_loader.dataset,
                                         ...
                                         device=device, args=args, pretrained_vae=pretrained_vae,  dataset_name=args.dataset)
        ...
        for i in range(len(test_selected_samples[0])):
            ...
            lbl_cluster, lbl_color, lbl_category, img_id, dataset_index = labels

            dir_label_str =  color_map.get(int(lbl_color), "Unk")

            caption_log = (f"Sample: {i + 1} | Index: {dataset_index} | ImgID: {img_id}\n"
                           f"Category: {lbl_category} | Cluster: {lbl_cluster} | Color: {dir_label_str}({lbl_color})\n"
        ...
        for i in range(NUM_VAES):
            for j in range(NUM_VAES):
                wandb.log(
                    {'IMG_Self&Cros_Gen_Cluster_prior/qz_m{}+pw_m{}'.format(i, j): wandb.Image(cg_imgs_cluster[i][j])},
                    step=epoch)
                if cg_imgs_cluster_denoised and cg_imgs_cluster_denoised[i][j] is not None:
                    wandb.log({'IMG_Self&Cros_Gen_Cluster_prior/qz_m{}+pw_m{}_denoised'.format(i, j): wandb.Image(
                        cg_imgs_cluster_denoised[i][j])}, step=epoch)
        ...
        if args.save_eval_images_root:
            ...
            save_recon_sequences_with_labels(recon_triess_to_table_cluster[0][0], labels_tensor,
                                             os.path.join(save_root, "noisy"), prefix="noisy_shared")
            den_entries = getattr(model, 'last_denoised_prior_entries', None)
            if den_entries and den_entries[0][0]:
                save_recon_sequences_with_labels(den_entries[0][0], labels_tensor, os.path.join(save_root, "denoised"),
                                                 prefix="denoised_shared")

        for i in range(NUM_VAES):
            for j in range(NUM_VAES):
                if i == 0 and j == 0:  # Img->Img
                    # Log Img->Img generations
                    images_to_log_00_grayscale = []
                    images_to_log_00_rgb = []
                    ...
                    for m in range(num_rows):  # N @ idmvae_CUB.py
                        for n in range(num_cols):  # num (B) @ idmvae_CUB.py
                            ...
                            caption_log = (f"[Img->Img]: generated_img + labels info\n"
                                           ...
                                           f"Category: {lbl_category} | Cluster: {lbl_cluster} | "
                                           f"Color: {dir_label_str}({lbl_color})\n"
                                           #    f"Input Image: {input_img}\n" # NG: will log the pixel values in number
                                           )

                            if lbl_color == 0:
                                images_to_log_00_grayscale.append(wandb.Image(generated_img, caption=caption_log))
                            else:
                                images_to_log_00_rgb.append(wandb.Image(generated_img, caption=caption_log))
                    ...
                    for n in range(num_cols):
                        ...
                        caption_log_with_input = (
                            f"[Img->Img]: Input Image #{n + 1} + {num_gen_samples} generations for it\n"
                            f"{caption_log_common}"
                            )
                        caption_log_without_input = (
                            f"[Img->Img]: {num_gen_samples} generations for Input Image #{n + 1}\n"
                            f"{caption_log_common}"
                            )
                    ...
                    # log m x n samples
                    wandb.log(
                        {"IMG_Gen_Cluster_prior_seperated/Single_Grayscale/Img->Img|qz_m{}+pw_m{}".format(i, j):
                             images_to_log_00_grayscale}, step=epoch)
                    wandb.log(
                        {"IMG_Gen_Cluster_prior_seperated/Single_Rgb/Img->Img|qz_m{}+pw_m{}".format(i, j):
                             images_to_log_00_rgb}, step=epoch)
                    wandb.log(
                ...
                if i == 0 and j == 1:  # Img->Cap
                    ...
                    captions_to_log_01_grayscale = []
                    captions_to_log_01_rgb = []
                    for m in range(num_rows):  # 3 (make_grid) or num_rows
                        for n in range(num_cols):
                            ...
                            if lbl_color == 0:
                                captions_to_log_01_grayscale.append(wandb.Image(input_img, caption=caption_log))
                            else:
                                captions_to_log_01_rgb.append(wandb.Image(input_img, caption=caption_log))

                    wandb.log(
                        {"IMG_Gen_Cluster_prior_seperated/Single_Grayscale/Img->Cap|qz_m{}+pw_m{}".format(i, j):
                             captions_to_log_01_grayscale}, step=epoch)
                    wandb.log(
                        {"IMG_Gen_Cluster_prior_seperated/Single_Rgb/Img->Cap|qz_m{}+pw_m{}".format(i, j):
                             captions_to_log_01_rgb}, step=epoch)

                if i == 1 and j == 0:  # Cap->Img
                    # Log Cap->Img generations
                    images_to_log_10_grayscale = []
                    images_to_log_10_rgb = []
                    img_gen_samples_groups = [[] for i in range(num_cols)]
                    img_gen_samples_groups_grids_with_input = []
                    img_gen_samples_groups_grids_without_input = []
                    for m in range(num_rows):
                        for n in range(num_cols):
                            ...
                            if lbl_color == 0:
                                images_to_log_10_grayscale.append(wandb.Image(generated_image, caption=caption_log))
                            else:
                                images_to_log_10_rgb.append(wandb.Image(generated_image, caption=caption_log))
                    ...
                    for n in range(num_cols):
                        ...
                        caption_log_with_input = (
                            f"[Cap->Img]: Input Caption #{n + 1} + {num_gen_samples} generations for it\n"
                            f"{caption_log_common}"
                            )
                        caption_log_without_input = (
                            f"[Cap->Img]: {num_gen_samples} generations for Input Caption {n + 1}\n"
                            f"{caption_log_common}"
                            )
                        img_gen_samples_groups_grids_with_input.append(
                            wandb.Image(
                    ...
                    wandb.log(
                        {"IMG_Gen_Cluster_prior_seperated/Single_Grayscale/Cap->Img|qz_m{}+pw_m{}".format(i, j):
                             images_to_log_10_grayscale}, step=epoch)
                    wandb.log(
                        {"IMG_Gen_Cluster_prior_seperated/Single_Rgb/Cap->Img|qz_m{}+pw_m{}".format(i, j):
                             images_to_log_10_rgb}, step=epoch)
                    wandb.log(
                ...
                if i == 1 and j == 1:  # Cap->Cap
                    ...
                    captions_to_log_11_grayscale = []
                    captions_to_log_11_rgb = []
                    for m in range(num_rows):  # 3 (make_grid) or num_rows
                        for n in range(num_cols):  # 3 (make_grid) or num_cols
                            ...
                            if lbl_color == 0:
                                captions_to_log_11_grayscale.append(wandb.Image(input_img, caption=caption_log))
                            else:
                                captions_to_log_11_rgb.append(wandb.Image(input_img, caption=caption_log))
                    wandb.log(
                        {"IMG_Gen_Cluster_prior_seperated/Single_Grayscale/Cap->Cap|qz_m{}+pw_m{}".format(i, j):
                             captions_to_log_11_grayscale}, step=epoch)
                    wandb.log(
                        {"IMG_Gen_Cluster_prior_seperated/Single_Rgb/Cap->Cap|qz_m{}+pw_m{}".format(i, j):
                             captions_to_log_11_rgb}, step=epoch)

        # Private: color
        cg_imgs_color, input_data_color, recon_triess_to_table_color = \
            cub_self_and_cross_modal_generation_eval(model,
                                                     test_selected_samples, num_cols, num_rows,
                                                     mode=CrossModalEvalForwardMode.PRIOR_CTRL,
                                                     condition_type='private')
        cg_imgs_color_denoised = getattr(model, 'last_denoised_prior_grids', None)
        for i in range(NUM_VAES):
            for j in range(NUM_VAES):
                if i == j:
                    wandb.log({'IMG_Self_Gen_Color_prior/qw_m{}+pz_m{}'.format(i, j): wandb.Image(
                        cg_imgs_color[i][j])}, step=epoch)
                    if cg_imgs_color_denoised and cg_imgs_color_denoised[i][j] is not None:
                        wandb.log({'IMG_Self_Gen_Color_prior/qw_m{}+pz_m{}_denoised'.format(i, j): wandb.Image(
                            cg_imgs_color_denoised[i][j])}, step=epoch)

        # plot color generations separately
        for i in range(NUM_VAES):
            for j in range(NUM_VAES):
                if i == 0 and j == 0:  # Img->Img
                    # Log Img->Img generations
                    ...
                    for m in range(num_rows):
                        for n in range(num_cols):
                            lbl_cluster, lbl_color, lbl_category, img_id, dataset_index \
                                = labels_list_of_tuples[n]
                            dir_label_str = color_map.get(int(lbl_color), "Unk")
                            generated_img = recon_triess_to_table_color[i][j][m][n]
                            caption_log = (f"[Img->Img]: generated_img + labels info\n"
                                           f"Sample: ({m + 1},{n + 1}) | Index: {dataset_index} | ImgID: {img_id}\n"
                                           f"Category: {lbl_category} | Cluster: {lbl_cluster} | "
                                           f"Color: {dir_label_str}({lbl_color})\n"
                    ...
                    wandb.log(
                        {"IMG_Gen_Color_prior_seperated/Single_Grayscale/Img->Img|qw_m{}+pz_m{}".format(i, j):
                             images_to_log_00_grayscale}, step=epoch)
                    wandb.log(
                        {"IMG_Gen_Color_prior_seperated/Single_Rgb/Img->Img|qw_m{}+pz_m{}".format(i, j):
                             images_to_log_00_rgb}, step=epoch)
                    wandb.log(
                        {"IMG_Gen_Color_prior_seperated/Group_w_Input/Img->Img|qw_m{}+pz_m{}".format(i, j):
                             img_gen_samples_groups_grids_with_input}, step=epoch)
                    wandb.log(
                        {"IMG_Gen_Color_prior_seperated/Group_wo_Input/Img->Img|qw_m{}+pz_m{}".format(i, j):
                             img_gen_samples_groups_grids_without_input}, step=epoch)
                    ...
                    for m in range(num_rows):  # 3 (make_grid) or num_rows
                        for n in range(num_cols):
                            ...
                            if lbl_color == 0:
                                ...
                                captions_to_log_11_rgb.append(wandb.Image(input_img, caption=caption_log))
                    wandb.log(
                        {"IMG_Gen_Color_prior_seperated/Single_Grayscale/Cap->Cap|qw_m{}+pz_m{}".format(i, j):
                             captions_to_log_11_grayscale}, step=epoch)
                    wandb.log(
                        {"IMG_Gen_Color_prior_seperated/Single_Rgb/Cap->Cap|qw_m{}+pz_m{}".format(i, j):
                             captions_to_log_11_rgb}, step=epoch)
        if args.save_eval_images_root:
            save_root_private = os.path.join(args.save_eval_images_root, f"epoch_{epoch}", "prior_private")
            save_images_with_labels(input_images, labels_tensor, os.path.join(save_root_private, "orig"),
                                    prefix="orig_private")
            save_recon_sequences_with_labels(recon_triess_to_table_color[0][0], labels_tensor,
                                             os.path.join(save_root_private, "noisy"), prefix="noisy_private")
            den_entries_priv = getattr(model, 'last_denoised_prior_entries', None)
            if den_entries_priv and den_entries_priv[0][0]:
                save_recon_sequences_with_labels(den_entries_priv[0][0], labels_tensor,
                                                 os.path.join(save_root_private, "denoised"), prefix="denoised_private")

        # rsample for posteriors, shuffled
        # Current call uses num=8 and N=8, bounded by selected test samples.
        cg_imgs_Shared_post, cg_imgs_Shared_post_ext, cg_imgs_Shared_post_ext_denoised = cub_self_and_cross_modal_generation_eval(
            model,
        ...
        for i in range(NUM_VAES):
            for j in range(NUM_VAES):
                wandb.log({'IMG_Self&Cros_Gen_Cluster_post/qz_m{}+qw_m{}_shuf'.format(i, j): wandb.Image(
                    cg_imgs_Shared_post[i][j])}, step=epoch)
                if cg_imgs_Shared_post_denoised and cg_imgs_Shared_post_denoised[i][j] is not None:
                    wandb.log({'IMG_Self&Cros_Gen_Cluster_post/qz_m{}+qw_m{}_shuf_denoised'.format(i, j): wandb.Image(
                        cg_imgs_Shared_post_denoised[i][j])}, step=epoch)
                if cg_imgs_Shared_post_ext and cg_imgs_Shared_post_ext[i][j] is not None:
                    wandb.log({'IMG_Self&Cros_Gen_Cluster_post_extended/qz_m{}+qw_m{}_shuf'.format(i, j): wandb.Image(
        ...
        for i in range(NUM_VAES):
            for j in range(NUM_VAES):
                if i == j:
                    wandb.log({'IMG_Self_Gen_Color_post/qw_m{}+qz_m{}_shuf'.format(i, j): wandb.Image(
                        cg_imgs_Private_post[i][j])}, step=epoch)
                    if cg_imgs_Private_post_denoised and cg_imgs_Private_post_denoised[i][j] is not None:
                        wandb.log({'IMG_Self_Gen_Color_post/qw_m{}+qz_m{}_shuf_denoised'.format(i, j): wandb.Image(
                            cg_imgs_Private_post_denoised[i][j])}, step=epoch)
                    if cg_imgs_Private_post_ext and cg_imgs_Private_post_ext[i][j] is not None:
                        wandb.log({'IMG_Self_Gen_Color_post_extended/qw_m{}+qz_m{}_shuf'.format(i, j): wandb.Image(
                            cg_imgs_Private_post_ext[i][j])}, step=epoch)
                    if cg_imgs_Private_post_ext_denoised and cg_imgs_Private_post_ext_denoised[i][j] is not None:
                        wandb.log({'IMG_Self_Gen_Color_post_extended/qw_m{}+qz_m{}_shuf_denoised'.format(i,
                                                                                                             j): wandb.Image(
                            cg_imgs_Private_post_ext_denoised[i][j])}, step=epoch)

        # rsample for posteriors, non-shuffled
        cg_imgs_Shared_post_nonshuf = cub_self_and_cross_modal_generation_eval(model,
                                                                               test_selected_samples, 8, 10,
        ...
        for i in range(NUM_VAES):
            for j in range(NUM_VAES):
                wandb.log({'IMG_Self&Cros_Gen_Shared_post_nonshuf/qz_m{}+qw_m{}'.format(i, j): wandb.Image(
                    cg_imgs_Shared_post_nonshuf[i][j])}, step=epoch)
                if cg_imgs_Shared_post_nonshuf_denoised and cg_imgs_Shared_post_nonshuf_denoised[i][j] is not None:
                    wandb.log({'IMG_Self&Cros_Gen_Shared_post_nonshuf/qz_m{}+qw_m{}_denoised'.format(i, j): wandb.Image(
...
def run_evaluation(epoch):
    ...
    if args.enable_latent_classification:
        ...
        if top_cluster_samples:
            for cluster_id, samples in top_cluster_samples.items():
                ...
                for sample_data in samples:
                    dir_label_str = color_map.get(int(sample_data['color_label']), "Unk")
                    caption = (f"GT_cluster: {sample_data['gt']} | "
                               ...
                               f"Dir: {dir_label_str}({int(sample_data['color_label'])})]")
                    images_to_log.append(wandb.Image(sample_data['image'], caption=caption))
                if images_to_log:
                    wandb.log({f"Predictions/Top-{len(images_to_log)}_Confidence_Cluster_{cluster_id}": images_to_log},
                              step=epoch)

        # Log all validation/test samples, grouped by cluster
        if 'prediction_data' in accuracies_lc_cluster:
            """
            prediction_data contains one entry per image-caption pair.
            The number of repeated entries per image is variable.
            """
            ...
            unique_positions = first_pair_position_per_image(
                cluster_data["dataset_indices"]
            )

            # for i in range(int(len(cluster_data['gts']) / 10)):  # Loop through all samples
            #     gt_cluster = int(cluster_data['cluster_labels'][
            #                          i * 10])  # Get the ground truth cluster label for the first image of the 10 duplicates
            ...
            #         grouped_ambiguous_samples[gt_cluster].append(
            #             i * 10)  # Store the index of the first image of the 10 duplicates
            #     else:
            #         grouped_samples[gt_cluster].append(
            #             i * 10)  # Store the index of the first image of the 10 duplicates
            for sample_index in unique_positions:
                gt_cluster = int(
                    cluster_data["cluster_labels"][sample_index]
                )

                if cluster_data["is_ambiguous"][sample_index]:
                    grouped_ambiguous_samples[gt_cluster].append(
                        sample_index
                    )
                else:
            ...
            for cluster_id, indices in grouped_samples.items():
                ...
                for i in indices:  # [:100]:
                    ...
                    dir_label_str = color_map.get(int(cluster_data['color_labels'][i]), "Unk")
                    ...
                    caption = (f"GT_cluster: {gt} | "
                               ...
                               f"Dir: {dir_label_str}({int(cluster_data['color_labels'][i])})]")
                ...
                if images_to_log:
                    wandb.log(
                        {f"Predictions/All-{len(images_to_log)}_valid_in_order_Cluster_{cluster_id}": images_to_log},
                        step=epoch)
            ...
            for cluster_id, indices in grouped_ambiguous_samples.items():
                ...
                # 5. Log the entire list of captioned images under a single key
                if images_to_log:
                    wandb.log({f"Predictions/Ambiguous-{len(images_to_log)}_Cluster_{cluster_id}": images_to_log},
                              step=epoch)
        ...
        accuracies_lc_color = linear_latent_classification_CUB_multi_labelTypes(
            model, test_time_loader, clf_lr_color, device, args, condition_type='private')

        for key, val in accuracies_lc_color.items():
            if key != 'prediction_data' and key != 'top_confidence_samples':  # Don't log the data dictionary itself
                tag = f"LatentClassAcc_color_clf/{key}"
                # log to wandb (will silently fail if offline)
                try:
                    wandb.log({tag: val}, step=epoch)
                except Exception:
        ...
        # NEW: Log top 5 confident color predictions
        top_color_samples = accuracies_lc_color.get('top_confidence_samples', {}).get('color', {})
        if top_color_samples:
            for color_id, samples in top_color_samples.items():
                color_label = color_map.get(color_id, "Unk")
                images_to_log = []
                for sample_data in samples:
                    gt_label = color_map.get(sample_data['gt'], "N/A")
                    pred_w_label = color_map.get(sample_data['pred_img_w'], "N/A")
                    pred_z_label = color_map.get(sample_data['pred_img_z'], "N/A")

                    caption = (f"GT_color: {gt_label} | "
                               ...
                               f"Cat: {int(sample_data['category_label'])}, "
                               f"Clu: {int(sample_data['cluster_label'])}, "
                               f"Dir: {color_label}({color_id})]")
                    images_to_log.append(wandb.Image(sample_data['image'], caption=caption))
                if images_to_log:
                    wandb.log({
                                  f"Predictions/Top-{len(images_to_log)}_Confidence_Color_{color_label}({color_id})": images_to_log},
                              step=epoch)
        ...
        if 'prediction_data' in accuracies_lc_color:
            ...
            # ADDED: Debug prints to check data shape
            print("--- DEBUG: Color Table Data ---")
            print(f"Number of images: {len(color_data['images'])}")
            print(f"Number of GT colors: {len(color_data['gts'])}")
            print(f"Number of predicted colors (m0_w): {len(color_data['preds']['m0_w'])}")
            print(f"Number of predicted colors (m0_z): {len(color_data['preds']['m0_z'])}")
            for key, value in color_data['preds'].items():
                print(f"Prediction '{key}' shape: {value.shape}")
            ...
            unique_pair_positions = first_pair_position_per_image(
                color_data["dataset_indices"]
            )

            for sample_index in unique_pair_positions:
                gt_color = int(
                    color_data["gts"][sample_index]
                )

                if color_data["is_ambiguous"][sample_index]:
                    grouped_ambiguous_samples[gt_color].append(
                        sample_index
                    )
                else:
                    grouped_samples[gt_color].append(
                        sample_index
                    )


            for color_id, indices in grouped_samples.items():
                ...
                for i in indices:  # [:100]:
                    ...
                    gt = color_data['gts'][i]
                    # Get predictions for the first image of the 10 duplicates
                    pred_img_w = color_data['preds']['m0_w'][i]
                    pred_img_z = color_data['preds']['m0_z'][i]
                    # Get confidence scores for the predictions
                    pred_img_w_conf = color_data['preds']['m0_w_confidence'][i]
                    pred_img_z_conf = color_data['preds']['m0_z_confidence'][i]

                    caption = (f"GT_color: {gt} | "
                               f"Img(W,+): {pred_img_w} ({pred_img_w_conf:.2f}) | "
                               f"Img(Z,-): {pred_img_z} ({pred_img_z_conf:.2f})\n"
                               f"[DataIdx: {int(color_data['dataset_indices'][i])}, "
                               f"ImgID: {int(color_data['img_ids'][i])}, "
                               f"Cat: {int(color_data['category_labels'][i])}, "
                               f"Clu: {int(color_data['cluster_labels'][i])}, "
                               f"Dir: {color_label_str}({color_id})]")
                ...
                if images_to_log:
                    wandb.log({
                                  f"Predictions/All-{len(images_to_log)}_valid_in_order_Color_{color_label_str}({color_id})": images_to_log},
                              step=epoch)

            # Log AMBIGUOUS samples, grouped by color
            for color_id, indices in grouped_ambiguous_samples.items():
                color_label_str = color_map.get(color_id, "Unk")
                images_to_log = []
                for i in indices:  # Log all ambiguous samples
                    image = color_data['images'][i]  # *10 Get the first image of the 10 duplicates
                    caption = (f"GT_color: {color_data['gts'][i]} | "
                               f"Img(W,+): {color_data['preds']['m0_w'][i]} ({color_data['preds']['m0_w_confidence'][i]:.2f}) | "
                               f"Img(Z,-): {color_data['preds']['m0_z'][i]} ({color_data['preds']['m0_z_confidence'][i]:.2f})\n"
                               f"[DataIdx: {int(color_data['dataset_indices'][i])}, "
                               f"ImgID: {int(color_data['img_ids'][i])}, "
                               f"Cat: {int(color_data['category_labels'][i])}, "
                               f"Clu: {int(color_data['cluster_labels'][i])}, "
                ...
                if images_to_log:
                    wandb.log({
                                  f"Predictions/Ambiguous-{len(images_to_log)}_Color_{color_label_str}({color_id})": images_to_log},
                              step=epoch)

            # <<< Log confidence histograms >>>
            wandb.log({
                "Confidence/Hist_Color_Img_W": wandb.Histogram(color_data['preds']['m0_w_confidence']),
    ...
    if args.enable_tSNE_UMAP:
        ...
        view0_w_color, view0_z_color_mis = visualize_latents_with_priors(
            model, model.vaes[0], model.encoders[0], test_time_image_loader, test_time_image_loader,
            "m0_img_w_color", "m0_img_z_color_mis", device=device, figure=3, condition_type='private',
            view_index=0, batch_size=args.batch_size, visualize_ratio=visualize_ratio,
            save_file=os.path.join(args.tSNE_save_dir,
